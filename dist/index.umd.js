(function(a,o){typeof exports=="object"&&typeof module<"u"?o(exports):typeof define=="function"&&define.amd?define(["exports"],o):(a=typeof globalThis<"u"?globalThis:a||self,o(a.InteractiveFormEmbedder={}))})(this,function(a){"use strict";typeof window<"u"&&(window.ifLayer=window.ifLayer||[]);const s=class s{constructor(e){this.widgets=new Map,e&&this.addWidget(e),this.processWidgetLayer()}static getInstance(){return s.instance||(s.instance=new s),s.instance}addWidget(e){const t=`${e.id}-${e.type}`;if(this.widgets.has(t)){console.warn(`Widget with id ${e.id} and type ${e.type} already exists`);return}this.widgets.set(t,e),this.initializeWidget(e)}removeWidget(e,t){const n=`${e}-${t}`,i=this.widgets.get(n);i&&(this.destroyWidget(i),this.widgets.delete(n))}reinitialize(){this.widgets.forEach(e=>{this.destroyWidget(e)}),this.widgets.forEach(e=>{this.initializeWidget(e)})}static createInlineWidget(e,t="614px",n="300px"){const i=document.createElement("iframe");i.src=`http://localhost:4200/${e}`,i.width=t,i.height=n,i.style.cssText=`
      max-width: 100%;
      width: ${t};
      height: ${n};
      border: none;
    ...
    `,d.addEventListener("mouseenter",()=>{d.style.background="#f0f0f0"}),d.addEventListener("mouseleave",()=>{d.style.background="transparent"});const r=this.createIframe(e.id,"614px","300px");i.appendChild(d),i.appendChild(r),t.addEventListener("click",()=>{i.style.display="block",t.style.animation="none"}),d.addEventListener("click",()=>{i.style.display="none",t.style.animation="ifScale 5s infinite"}),document.body.appendChild(t),document.body.appendChild(i)}createPopUpEmbed(e){const t=e.timeout?e.timeout*1e3:3e3;setTimeout(()=>{const n=document.createElement("div");n.style.cssText=`
      ...
      `;const d=document.createElement("button");d.innerHTML="&times;",d.style.cssText=`
        ...
        border-radius: 50%;
        transition: background-color 0.2s ease;
      `,d.addEventListener("mouseenter",()=>{d.style.background="#f0f0f0"}),d.addEventListener("mouseleave",()=>{d.style.background="transparent"});const r=this.createIframe(e.id,"614px","300px");i.appendChild(d),i.appendChild(r),n.appendChild(i);const c=document.createElement("style");c.textContent=`
        @keyframes fadeIn {
          from { opacity: 0; }
          to { opacity: 1; }
        }
      ...
      height: ${n};
      border: none;
    `,i}};s.instance=null;let o=s;(function(){typeof window<"u"&&(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{new o}):new o)})(),a.Embedder=o,Object.defineProperty(a,Symbol.toStringTag,{value:"Module"})});
